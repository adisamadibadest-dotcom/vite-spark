...
router.get("/gold-price", async (_req: Request, res: Response) => {
  const quote = await Promise.any([
    requireQuote(fromGoldApi()),
    requireQuote(fromMetalsLive()),
  ]).catch(() => null);
  if (!quote) {
    res.status(502).json({ error: "Unable to fetch gold price" });
    return;
  }
...
type ChartAnnotations = z.infer<typeof AnnotationSchema>;
