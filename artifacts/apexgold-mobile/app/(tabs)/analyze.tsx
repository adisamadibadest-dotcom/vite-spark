...
export default function AnalyzeScreen() {
  ...
  const saveToJournal = async () => {
    if (!result || !user) {
      Alert.alert("Sign in required", "Please sign in to save trades to your journal.");
      return;
    }
    setSaving(true);
    await Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    try {
      const imageDataUrl = imageBase64 ? `data:${mimeType};base64,${imageBase64}` : null;
      ...
      Alert.alert("Save failed", err instanceof Error ? err.message : "Could not save to journal.");
      await Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error);
    } finally {
      setSaving(false);
    }
    ...
    >
      ...
      {result && (
        <>
          ...
          {/* Save to Journal */}
          <Pressable
            style={({ pressed }) => [s.saveBtn, saved && s.saveBtnSaved, (saving || saved) && s.saveBtnDisabled, pressed && { opacity: 0.8 }]}
          ...
          >
            {saving ? (
              ...
              <Ionicons name={saved ? "checkmark-circle" : "bookmark-outline"} size={16} color={saved ? colors.bullish : colors.background} />
            )}
            <Text style={[s.saveBtnText, saved && { color: colors.bullish }]}>
              {saved ? "Saved to Journal" : saving ? "Saving…" : "Save to Journal"}
            </Text>
          </Pressable>

          {/* Re-analyze */}
          <Pressable style={s.resetBtn} onPress={() => { setResult(null); setImageUri(null); setImageBase64(null); setSaved(false); }}>
...
function styles(colors: any) {
  return StyleSheet.create({
    ...
    saveBtn: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, backgroundColor: colors.gold, borderRadius: 12, height: 48 },
    saveBtnSaved: { backgroundColor: colors.card, borderWidth: 1, borderColor: colors.bullish + "60" },
    saveBtnDisabled: { opacity: 0.7 },
    saveBtnText: { fontSize: 15, fontWeight: "700" as const, color: colors.background },
