...
const AnnotationSchema = z.object({
  ...
  markers: z.array(z.object({
    type: z.enum(["bos", "choch", "liquidity", "sweep"]).transform(
      (v) => (v === "sweep" ? "liquidity" : v) as "bos" | "choch" | "liquidity"
    ),
...
function extractPricesFromAnalysis(annotation: ChartAnnotations): number[] {
  const text = [
    ...
    ...(annotation.setup
      ? [annotation.setup.entry, annotation.setup.stopLoss, ...annotation.setup.takeProfits]
      : []),
  ].join(" ");
...
function completeTradeSetup(annotation: ChartAnnotations): ChartAnnotations {
  ...
  if (
    ...
    (current.entry || current.entryZone) &&
    current.stopLoss &&
    current.takeProfits.length >= 3 &&
    current.riskReward
  ) {
    return annotation;
  }
  const hasStructure = annotation.markers.some(
    (marker) => marker.type === "bos" || marker.type === "choch"
  );
  const hasActionableZone = annotation.zones.length > 0;
  if (
    annotation.bias === "neutral" ||
    annotation.confidence < 50 ||
    (!hasStructure && !hasActionableZone)
  ) {
    return {
      ...annotation,
      setup: {
  ...
  const entry = isLong
    ? prices[Math.min(1, prices.length - 1)]
    : prices[Math.max(prices.length - 2, 0)];
  ...
  const nearestTarget = isLong
    ? prices.find((price) => price > entry + risk * 2.5)
    : [...prices].reverse().find((price) => price < entry - risk * 2.5);
  ...
  const takeProfits = [...(current?.takeProfits ?? []), ...fallbackTakeProfits]
    .filter(Boolean)
    .slice(0, 3);
  return {
    ...annotation,
    setup: {
      ...
      direction,
      tradeType:
        current?.tradeType && current.tradeType !== "none"
          ? current.tradeType
          : inferTradeType(annotation),
      confidence:
        current?.confidence && current.confidence > 0
          ? current.confidence
          : Math.max(60, Math.min(90, annotation.confidence - 5)),
      ...
      riskReward:
        current?.riskReward ||
        `1:${(Math.abs(tp2 - entry) / risk).toFixed(1)}`,
      rationale:
        current?.rationale ||
        `${isLong ? "Bullish" : "Bearish"} continuation setup from the identified institutional zone after structure confirmation and liquidity context.`,
    },
  };
}
...
- Zone types: "ob", "demand", "supply", "fvg", "support", "resistance"
- Marker types: "bos", "choch", "liquidity"
- summary + reasoning MUST match what you drew.
- TRADE SETUP is MANDATORY when structure + zone confluence is visible.
- When valid=true: entry (or entryZone), stopLoss, exactly TP1/TP2/TP3, riskReward vs TP2, tradeType, confidence 0-100, and rationale.
- Prices must be read off the visible chart axis.
...
function extractJsonObject(text: string): unknown {
  ...
  if (start === -1 || end === -1 || end <= start)
    throw new Error("AI returned unreadable chart analysis.");
...
export async function handleAnalyzeChart(body: {
  ...
  try {
    ...
    console.log(`[analyze-chart] received image, b64Len=${imageBase64.length}, mimeType=${mimeType}`);

    if (imageBase64.length > 3_800_000) {
      return {
        status: 413,
        body: { error: "Image is too large. Please upload a smaller screenshot." },
      };
    }
    ...
    const msg = err instanceof Error ? err.message : "AI error";
    const status = /429/.test(msg) ? 429 : /402/.test(msg) ? 402 : 500;
    console.error(`[analyze-chart] returning error status=${status}: ${msg}`);
    return { status, body: { error: msg } };
  }
}
